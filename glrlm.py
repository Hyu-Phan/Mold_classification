...
def glrlm_features(f, mask, Ng=256):
    ...
    angel = [0, 45, 90, 135]
    labels = [label + "_" + str(ang) for label in labels for ang in angel]
    ...
    features = np.zeros(44,np.double)
    features[0:4] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.divide, rlmatrix, (J*J)), axes=(0, 1))[0, 0])/S)
    features[4:8] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.multiply, rlmatrix, (J*J)), axes=(0, 1))[0, 0])/S)
    features[8:12] = ((np.apply_over_axes(np.sum, (G*G), axes=(0, 1))[0, 0])/S)
    features[12:16] = ((np.apply_over_axes(np.sum, (R*R), axes=(0, 1))[0, 0])/S)
        
    gray_level, run_length,_ = rlmatrix.shape
    num_voxels = gray_level * run_length
    features[16:20] = (S/num_voxels)
        
    features[20:24]= ((np.apply_over_axes(np.sum, _apply_over_degree(np.divide, rlmatrix, (I*I)), axes=(0, 1))[0, 0])/S)
    features[24:28] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.multiply, rlmatrix, (I*I)), axes=(0, 1))[0, 0])/S)
    features[28:32] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.divide, rlmatrix, (I*I*J*J)), axes=(0, 1))[0, 0])/S)
        
    temp = _apply_over_degree(np.multiply, rlmatrix, (I*I))
    features[32:36] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.divide, temp, (J*J)), axes=(0, 1))[0, 0])/S)
        
    temp = _apply_over_degree(np.multiply, rlmatrix, (J*J))
    features[36:40] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.divide, temp, (J*J)), axes=(0, 1))[0, 0])/S)
    features[40:] = ((np.apply_over_axes(np.sum, _apply_over_degree(np.multiply, rlmatrix, (I*I*J*J)), axes=(0, 1))[0, 0])/S)
